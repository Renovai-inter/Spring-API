...
public class EnderecoController {
    ...
    public ResponseEntity<List<EnderecoResponse>> listar() {
        return ResponseEntity.ok(repository.findAll().stream()
                .map(e -> new EnderecoResponse(e.getEnderecoId(), e.getCep(), e.getLogradouro(),
    ...
    public ResponseEntity<EnderecoResponse> buscarPorId(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(new EnderecoResponse(e.getEnderecoId(), e.getCep(), 
                e.getLogradouro(), e.getNumero(), e.getComplemento(), e.getBairro(), e.getCidade()));
    }
    ...
    public ResponseEntity<EnderecoResponse> criar(@RequestBody @Valid EnderecoRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new EnderecoResponse(saved.getEnderecoId(), saved.getCep(), 
                        saved.getLogradouro(), saved.getNumero(), saved.getComplemento(),
                        saved.getBairro(), saved.getCidade()));
    }
    ...
    public ResponseEntity<EnderecoResponse> atualizar(
        ...
        return ResponseEntity.ok(new EnderecoResponse(saved.getEnderecoId(), saved.getCep(),
                saved.getLogradouro(), saved.getNumero(), saved.getComplemento(),
                saved.getBairro(), saved.getCidade()));
    }
