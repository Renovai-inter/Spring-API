...
public class CargoController {
    ...
    public record CargoRequest(
        @NotBlank @Size(max = 10, message = "Cargo deve ter no máximo 10 caracteres") String cargo) {}
    public record CargoResponse(Integer cargoId, String cargo) {}
    ...
    public ResponseEntity<List<CargoResponse>> listar() {
        return ResponseEntity.ok(
                repository.findAll().stream()
                        .map(c -> new CargoResponse(c.getCargoId(), c.getCargo()))
                        .toList());
    }
    ...
    public ResponseEntity<CargoResponse> atualizar(
            @PathVariable Integer id, @RequestBody @Valid CargoRequest request) {
        Cargo c = repository.findById(id)
                .orElseThrow(() -> new RecursoNaoEncontradoException("Cargo", id));
        c.setCargo(request.cargo());
        Cargo saved = repository.save(c);
        return ResponseEntity.ok(new CargoResponse(saved.getCargoId(), saved.getCargo()));
    }
